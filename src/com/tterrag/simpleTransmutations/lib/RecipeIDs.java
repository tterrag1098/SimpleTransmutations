package com.tterrag.simpleTransmutations.lib;

public class RecipeIDs {

	public static boolean woodToObsidian = true;
	public static boolean woodToWood = true;
	public static boolean ironToGold = true;
	public static boolean goldToDiamond = true;
	public static boolean ironToEnderpearl = true;
	public static boolean inkTransmutation = true;
	public static boolean allowBonemealTransmutation = false;
	public static boolean allowInkSacTransmutation = false;
	public static boolean allowLapisTransmutation = false;

	public static final String WOOD_TO_OBSIDIAN_KEY = "Wood_to_Obsidian";
	public static final String WOOD_TO_WOOD_KEY = "Wood_to_Wood";
	public static final String IRON_TO_GOLD_KEY = "Iron_to_Gold";
	public static final String GOLD_TO_DIAMOND_KEY = "Gold_to_Diamond";
	public static final String IRON_TO_ENDERPEARL_KEY = "Iron_to_Enderpearl";
	public static final String INK_TRANSMUTATION_KEY = "Ink_Transmutation";
	public static final String ALLOW_BONEMEAL_KEY = "Allow_Bonemeal_Transmute";
	public static final String ALLOW_INK_KEY = "Allow_Ink_Transmute";
	public static final String ALLOW_LAPIS_KEY = "Allow_Lapis_Transmute";

}
